...
class Decoder_clean(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # Gumbel-Softmax 选择专家
        logits = self.expert_selector(x)
        expert_weights = gumbel_softmax_sample(logits, tau=self.tau)

        # 计算专家输出
        expert_outputs = torch.stack([expert(x) for i, expert in enumerate(self.experts)], dim=1)
        x = (expert_outputs * expert_weights.unsqueeze(2)).sum(dim=1)
